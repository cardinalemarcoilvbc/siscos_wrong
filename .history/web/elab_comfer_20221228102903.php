...
include __DIR__ . '/include/esolver.php';
...
$p = new ProgressBar();
if (!empty($_POST)) {
  //  ($_POST['state']) == 0 ? die("non è stato selezioanto il codice ATECO") : 1;
...
//die($_POST['tper']);
$eleb = ($esolver->elefile(1));
$eleb2 = ($esolver->elefile(2));
//var_dump($eleb);
//var_dump($eleb2);
...
foreach ($elef as $t) {
    if (count($elef) == 1) {
     ...
     echo    str_pad(basename($t),6, STR_PAD_BOTH) . '<br></td></tr><tr><td>';
     ...
     */
   // echo basename($t);
   $riga = $esolver->creafile_comfer(basename($t));
   //$ftp=$esolver->transmit_file_comfer(basename($t));

 //   $riga = $riga . $pfs->creaditta_File(basename($t), 
  //  $contaid, $_POST['state'],$_POST['tper']);
