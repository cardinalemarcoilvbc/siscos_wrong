...
class ls
{
 ...
 public function elefile_amz($dir)
    {
        ini_set('display_errors', '1');
        ini_set('display_startup_errors', '1');
        error_reporting(E_ALL);
        ...
        // echo  str_replace('include','',__DIR__).$ini_array['percorsi']['oripath'];
        //   echo  (str_replace('include','',__DIR__).$ini_array['percorsi']['oripath']);
        if (!is_dir(str_replace('include', '', __DIR__) . $ini_array['percorsi']['oripath_amz'])
        ) {

            mkdir('/app/web/toelab_amz/', 0777);
            // echo $this->folder_exist('/app/web/toelab/');
            if (!is_dir('/app/web/toelab_amz/')) {
        ...
        $search = $search . '}';
        $search = str_replace(',}', '}', $search);
        $oripath = glob('.\\' . $ini_array['percorsi']['oripath_amz'] . '*.' . $search, GLOB_BRACE);
        // glob($ini_array['percorsi']['oripath'] . $ext);
        //var_dump($oripath);
        //var_dump($ini_array['percorsi']['oripath']);
        $directory = new DirectoryIterator(dirname(__FILE__));
        ...
        foreach ($oripath as $f) {
            if (!file_exists($di . $ini_array['percorsi']['procfiles_amz'] . (basename($f)))) {
                copy($f, $di . $ini_array['percorsi']['toelab_amz'] . (basename($f)));
            } else {
            }
        }
        $res = [];
        if ($dir == 1) {
            $lpath = glob($di . $ini_array['percorsi']['toelab_amz'] . '*.' . $search, GLOB_BRACE);
            foreach ($lpath as $f) {
                if (!file_exists($di . $ini_array['percorsi']['procfiles_amz'] . (basename($f)))) {
                    array_push($res, basename($f));
                }
            ...
            $lpath = glob($di . $ini_array['percorsi']['procfiles_amz'] . '*.' . $search, GLOB_BRACE);
    ...
    public function localelab_amz()
    {
        $ini_array = parse_ini_file("config.ini", true/* will scope sectionally */);
        $ext = $ini_array['Parametri']['estensione'];
        $directory = new DirectoryIterator(dirname(__FILE__));
        $di = str_replace('include', '', $directory->getPath());
        $lpath = glob($di . $ini_array['percorsi']['toelab_amz'] . $ext);
        foreach ($lpath as $f) {
        }
    }
  ...
  public function creafile_amz($rows)
    {
        $ini_array = parse_ini_file("config.ini", true/* will scope sectionally */);
        $di = str_replace('include', '', __DIR__);
        //  echo $di.'<br>';
       ...
       $nfile = $di . $ini_array['Parametri']['Nomecorr_amz'] . '.csv';
        $tmpfile = fopen($nfile, "w")
        or $this->extremesave($rows['corrispettivi']);

        fwrite($tmpfile, $rows['corrispettivi']);
        fclose($tmpfile);

        $nfile = $di . $ini_array['Parametri']['Nometf_amz'] . '.csv';
$tmpfile = fopen($nfile, "w")
or $this->extremesave($rows['fattura']);
fwrite($tmpfile, $rows['fattura']);
fclose($tmpfile);

 
        if (strlen($rows['corrispettivi']|| $rows['fattura']) > 10) {
