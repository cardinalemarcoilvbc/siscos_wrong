  ...
  public function creafile_amz($rows, $tipo = null)
    {
        ...
        //  echo $di.  $ini_array['Parametri']['NomeOut']  . '.csv';
        if ($tipo == null) {
            $nfile = $di . $ini_array['Parametri']['NomeOut_amz'] . '.csv';
        } elseif ($tipo == 2) {
            $nfile = $di . $ini_array['Parametri']['NomeANA_amz'] . '.csv';
        }
        $tmpfile = fopen($nfile, "w")
        or $this->extremesave($rows);
        ...
        if ($tipo == null) {
            $webf = fopen($di . $ini_array['Parametri']['NomeOut_amz'] . date('m-d-Y_hia') . '.csv', "w") or
            $this->extremesave($rows);
