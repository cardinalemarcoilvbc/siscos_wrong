  ...
  public function creafile_amz($rows)
    {
       ...
       $nfile = $di . $ini_array['Parametri']['Nomecorr_amz'] . '.csv';
        $tmpfile = fopen($nfile, "w")
        or $this->extremesave($rows[0]['corrispettivi']);
...
$tmpfile = fopen($nfile, "w")
or $this->extremesave($rows[0]['fattura']);
fwrite($tmpfile, $rows[0]['fattura']);
fclose($tmpfile);

 
        if (strlen($rows[0]['corrispettivi']|| $rows[0]['fattura']) > 10) {
