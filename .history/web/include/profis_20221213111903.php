...
//EOT.PHP_EOL;
    $i=0;               
foreach($tmp_arr -> value){
//$tmp_arr[$i]->ltrim(rtrim($tmp_arr[$i]));
$i++;
}
...
//var_dump($c2);
                    ...
                    var_dump(floatval((str_replace('.', '', $tmp_arr['9']))));
                    var_dump(ltrim(rtrim(str_replace(',', '.', (str_replace('.', '', $tmp_arr['9']))))));
                    $cazzo = ltrim(rtrim(str_replace(',', '.', (str_replace('.', '', $tmp_arr['9'])))));
                    var_dump(floatval($cazzo));
                    var_dump(floatval(preg_replace("/[^-0-9\.]/", "", $tmp_arr['10'])));
                    var_dump(floatval(preg_replace("/[^-0-9\.]/", "", ($tmp_arr['11']))));

                    $tmp_arr['3'] = str_replace('.', '', $tmp_arr['3']);

                    $tmp_arr['9'] = str_pad(ltrim(rtrim(str_replace('.', '', $tmp_arr['9']))), 17, ' ', STR_PAD_LEFT);
                    $tmp_arr['10'] = str_pad(ltrim(rtrim(str_replace('.', '', $tmp_arr['10']))), 17, ' ', STR_PAD_LEFT);
                    $tmp_arr['11'] = str_pad(ltrim(rtrim(str_replace('.', '', $tmp_arr['11']))), 17, ' ', STR_PAD_LEFT);

                    var_dump($tmp_arr['9']);
                    var_dump($tmp_arr['10']);
                    var_dump($tmp_arr['11']);

                    $tmp_arr['9'] = str_pad(str_replace('.', ',', $tmp_arr['9']), 17, ' ', STR_PAD_LEFT);

                    $tmp_arr['10'] = str_pad(str_replace('.', ',', $tmp_arr['10']), 17, ' ', STR_PAD_LEFT);
                    $tmp_arr['11'] = str_pad(str_replace('.', ',', $tmp_arr['11']), 17, ' ', STR_PAD_LEFT);

                    $anno_contabile = substr(str_replace('.', '', $tmp_arr['3']), -4);

//$var10_niva=floatval($tmp_arr['9']) -(floatval($tmp_arr['11']) ?? 0);
//$var11_niva=floatval($tmp_arr['10'])-(floatval($tmp_arr['11']) ?? 0);
